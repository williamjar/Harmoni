...
module.exports = class documentDao extends Dao {
    ...
    getOne(callback, organizerID) {
        super.query('SELECT * FROM organizer JOIN contact ON organizer.contactID = contact.contactID JOIN picture ON organizer.pictureID = picture.pictureID WHERE organizer.organizerID = ?', [organizerID], callback);
    }
